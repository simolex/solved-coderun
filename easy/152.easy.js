...
const moveByBack = [
    (v) => (v % 3 === 0 ? v / 3 : undefined),
    (v) => (v % 2 === 0 ? v / 2 : undefined),
    (v) => v - 1,
];
...
rl.on("line", (line) => {
    ...
    while (!findedNode) {
        ...
        findedNode = newFront.nextStepAndFind(
            front[front.length - 1].vertex,
            back[back.length - 1].vertex
        );
        front.push(newFront);
        if (findedNode) break;

        const newBack = new Layer(moveByBack);
        findedNode = newBack.nextStepAndFind(
            back[back.length - 1].vertex,
            front[front.length - 1].vertex
        );
        back.push(newBack);
